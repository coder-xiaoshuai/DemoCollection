...
public class DisplayView extends ViewGroup{
    ...
    /*
    * 若为true在loop模式下点击view直接展开，list模式下相应子view点击
    * */
    ...
    * */
    ...
    public DisplayView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray typedArray=context.obtainStyledAttributes(attrs,R.styleable.DisplayView,defStyleAttr,0);
        mBtnSpacing=typedArray.getDimensionPixelSize(R.styleable.DisplayView_btn_spacing,dpToPx(context,10));
        int layoutCollapse = typedArray.getResourceId(R.styleable.DisplayView_collapse_layout, R.layout.layout_collapse_button);
        int layoutExpand = typedArray.getResourceId(R.styleable.DisplayView_expand_layout, R.layout.layout_expand_button);
        mClickLoopToExpand = typedArray.getBoolean(R.styleable.DisplayView_click_loop_to_expand, false);
     ...
     *
     * @param views
     * @param keepChildCount 保留在xml里面的添加的子view数量，因为要除去mBtnCollapse/mBtnExpand，所以第三个开始计算
     */
    ...
    /**
     * 在已经存在menu item的时候调用，不会清除已有的item
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        if(mDisplayMode==DisplayMode.LIST){
            ...
            for (int i=0;i<getChildCount();i++){
                View child=getChildAt(i);
                if(child.getVisibility()==GONE||child==mBtnExpand)continue;
                //DisplayMode.Loop模式时，onlayout会把menu进行横坐标的偏移，这里把坐标回置
            ...
            int centerHorzontalX=(right-left)/2;
            int nextY=getPaddingTop();
            ...
            for(int i=0;i<getChildCount();i++){
                ...
                if(child==mBtnExpand||child==mBtnCollapse){
                    int l=centerHorzontalX-child.getMeasuredWidth()/2;
                    int t=nextY;
                    int r=l+child.getMeasuredWidth();
                    int b=t+child.getMeasuredHeight();
                    child.layout(l,t,r,b);
                    ...
                    //DisplayMode.LIST 模式时，动画会把menuView 进行纵坐标的偏移，透明度设置，这里把坐标，透明度回置
                    child.setAlpha(1);
                    child.setTranslationX(0);
                    //除去第一个item设置到可见位置，其余的位置设置到控件右边不可见位置
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if(mDisplayMode==DisplayMode.LIST){
            //DisplayMode.LIST相当于LinearLayout
            for(int i=0;i<getChildCount();i++){
                View child=getChildAt(i);
                if (child.getVisibility()==GONE)continue;
            ...
            //DisplayMode 高度只包括 最高的子view的高度+mBtnExpand的高度
